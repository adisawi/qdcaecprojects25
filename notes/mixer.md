...
Gate receives Vosc through Cc; this modulates the channel conductivity.
  Vin, applied to the drain directly, modulates Vds
...
Desired Output
We typically filter and keep only the LSB.
Easier to amplify, filter, and demodulate than the original Vin signal.
...
May include:
  ...
  Residual Vosc and Vin components.
  Higher-order harmonics.

However, for this use case:
  These components are typically well-separated in frequency.
  They can be effectively filtered and do not interfere with desired IF and QF output.
...
Other types of mixers would be ones that use differential or balanced mixer. As this circuit is single-ended, it may have unintentional problems with the output such as DC component, residual components directly from the inputs, higher-order harmonics or some asymmetries. For our purposes, this does not pose too much of an issue, as these incorrect frequencies are generally far enough away from our required frequencies to filter them out.
